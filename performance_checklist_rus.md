...
<p>
    ...
    OCSP и сжатие HPACK? Не стоит так же забывать
    о подсказках ресурсов, клиентских подсказках
    и сдерживании CSS— и это не говоря уже о IPv6, HTTP/2 и сервис воркерах.
</p>
<p>
    Давайте вспомним дни, когда производительность часто откладывалась на второй план.
    Дело доходило до нее аж в конце проекта, и в большинстве случаев это сводилось к
    минификации, конкатенации и оптимизации содержимого, и, возможно, парой
    ...
    действительно важна, нужно учитывать, как может отразится на производительности
    то или иное решение еще в момент создания дизайна. Производительность нужно
    постоянно замерять, отслеживать, и улучшать, и вечно-растущая сложность
    веб-технологий все чаще бросает разработчику вызов, делая невозможным
    отслеживание метрик, так как метрики будут значительно отличаться от устройства к устройству,
    от браузера к браузеру, будут зависить от сетевого протокола, типа сети и
...
<p>
    Текущие микро-оптимизации — это отличная вещь,
    чтобы удерживать Ваш продукт в строю, но гораздо
    важнее предопределить какие цели стоят перед Вами  и
    не забывать о них, ведь они которые будут влиять
    на все решения, принимаемые на протяжении всего процесса.
    Существует множество разнообразных моделей, и приведенные ниже
    – могут варьироваться, но главное — расставить в своем
    проекте правильные приоритеты.
</p>
...
<p>
    ...
    что пользователь ощущал что Ваш вебсайт работает быстрее других, нужно быть как-минимум
    на 20% быстрее.  Не столь важно время полной загрузки и отображения страницы,
    как условные метрики, например, время начала рендера страницы,
    ...
    чтобы страница отобразила свой преймущественный контент) и
...
<p>
    ...
    тестированием. Соберите информацию, составьте таблицу, сожмите ее на 20%
...
<p>
    ...
    как анимация, лучшим решением будет не нагромаждать там, где есть возможность такая возможность, а там где ее нет - обходится самым минимумом.
    Более того, каждый кадр анимации должен сменятся менее чем
    за 16 миллисекунд, тем самым Вы достигнете заветных 60 fps
    (1 секунда ÷ 60 = 16.6 миллисекунд) —
    а еще лучше – уложиться в пределы 10 миллисекунд.
    Вашему браузеру ведь нужно время что бы отрисовать новый кадр на экране,
    так что постарайтесь что бы исполнение Вашего кода завершилось
    в течении 16.6 миллисекунд.
...
<p>
    ...
    3G подключения. Если Ваши показатели слегка перевалили за это значения –
...
<p>
    ...
    и Вы не ощущаете проблем в процессе использования инструментов сборки – у Вас должно быть все хорошо.
</p>
<h3>
...
<p>
    ...
    в качестве ведущего принцип архитектуры вашего front-end’а. Сначала спроектируйте и реализуйте базовые возможности,
    ...
    со слабым браузером и посредственным интернетом, только тогда Вы можете быть уверены, что он будет отлично
...
<p>
    ...
    динамического контента тоже. Так что не обязательно ограничиваться в использовании CDN только для статического контента.
...
<p>
    Было бы мудро, для начала ознакомиться с чем Вам придется работать. Проведите «инвентаризацию» всех своих ресурсов и контента
    (JavaScript, изображения, шрифты, внешние скрипты, и “увесистые” модули на странице, такие как карусели, сложная инфографика, и мультимедийный контент),
...
<p>
    ...
    <i><a href="https://aerotwist.com/blog/when-everything-is-important-nothing-is/" target="_blank">
        ...
        подразумевает использование отрисовки страницы на сервере что бы получить первую значимую картинку максимально быстро,
        а также напишите хоть минимальный JavaScript, чтобы привести время интерактивности ближе к времени отрисовки первой значимой картинки.
    </i>
...
<p>
    ...
    к минимум ненужные экспорты кода</a>, и <a href="https://github.com/giakki/uncss" target="_blank">UnCSS </a>
...
<p>
    и еще раз, <a href="https://www.youtube.com/watch?v=yURLTwZ3ehk" target="_blank">обработка контента через HTTP/2</a> требует
    больших доработок и сильно отличается от того, как вы обрабатывали ваши активы раньше.
    Вам придется найти наилучший баланс между упаковкой всех модулей в один и загрузкой множества
    ...
    их в параллели. В отличии от одного файла в случае неудачной загрузки не придется
    выгружать полностью все стили и скрипты заново. <br>
    С другой стороны, <a href="http://engineering.khanacademy.org/posts/js-packaging-http2.htm" target="_blank">упаковка всех файлов в один </a>все еще имеет право на существование, потому что
    ...
    подобные изменения за <a href="https://www.chromium.org/developers/design-documents/inter-process-communication" target="_blank">меж-процессные коммуникации </a>
...
">представили миру Brotli</a>, новый формат данных без потерь,
    ...
    ресурсов, которые не очень-то меняются, файлов, которые, были созданы, чтобы однажды
...
<p>
    ...
    каждую ссылку. Убедитесь, что Вы знаете, как проанализировать и улучшать
...
<p>
    ...
    на которых ориентирован Ваш продукт, чтобы в будущем не столкнуться с
...
<p>
    ...
    порядочно времени. А что если бы у Вас был лишь 1 час что бы
    добиться значительных улучшений? Какое бы решение приняли Вы?
    Давайте сузим список до десятки самых просто-достижимых целей.
...
<ol>
    ...
    <li>Подготовьте критичный CSS для основны интерфейса, и включите их в <code>
    ...
    <li>Если представляется возможным, кэшируйте содержимое, например, шрифты,
        стили, скрипты и изображения, а вообще, чем больше – тем лучше — в кэши сервис воркеров.</li>
</ol>
...
<p>
    ...
    Не забывайте скачать PDF файл с чеклистом, подготовленным к печати для
    Ваших нужд:
</p>
<ul>
